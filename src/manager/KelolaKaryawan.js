...
export default function KelolaKaryawan() {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      const res = await fetch("https://absensi-backend-production-a2fe.up.railway.app/api/users", {
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`https://absensi-backend-production-a2fe.up.railway.app/api/users/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleSave = async () => {
    ...
    const url = isAdding
      ? "https://absensi-backend-production-a2fe.up.railway.app/api/users"
      : `https://absensi-backend-production-a2fe.up.railway.app/api/users/${editData._id}`;
