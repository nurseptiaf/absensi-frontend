...
export default function LeaveOfAbsence() {
  ...
  const fetchLeaves = useCallback(async () => {
    try {
      const response = await axios.get("https://absensi-backend-production-a2fe.up.railway.app/api/leave/my", {
        headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("https://absensi-backend-production-a2fe.up.railway.app/api/leave/apply", formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
