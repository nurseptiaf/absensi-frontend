...
export default function Dashboard() {
    ...
    const fetchAttendanceStatus = async () => {
      ...
      try {
        const res = await fetch(
          "https://absensi-backend-production-a2fe.up.railway.app/api/attendance/records",
          {
    ...
    const fetchNotifications = async () => {
      try {
        const [attendanceRes, leaveRes, reimburseRes] = await Promise.all([
          fetch("https://absensi-backend-production-a2fe.up.railway.app/api/attendance/records", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("https://absensi-backend-production-a2fe.up.railway.app/api/leave/my", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("https://absensi-backend-production-a2fe.up.railway.app/api/reimbursement/my", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleCheckIn = async () => {
    ...
    setMessage("");
    try {
      const res = await fetch("https://absensi-backend-production-a2fe.up.railway.app/api/attendance/checkin", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const handleCheckOut = async () => {
    ...
    setMessage("");
    try {
      const res = await fetch("https://absensi-backend-production-a2fe.up.railway.app/api/attendance/checkout", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
